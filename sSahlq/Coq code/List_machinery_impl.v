...
(* Similar to alt_Ip but with list. *)
...
Lemma alt_Ip_eq : forall (W : Set) (Ip : predicate -> W -> Prop) (pa1 pa2 : W -> Prop)
                                 (P : predicate),
     alt_Ip (alt_Ip Ip pa2 P) pa1 P 
                    = alt_Ip Ip pa1 P.
Proof.
  intros.
...
Lemma alt_Ip_switch : forall (W : Set) (Ip : predicate -> W -> Prop) (pa1 pa2 : W -> Prop)
                                 (n m : nat),
     ~(n = m) -> (alt_Ip (alt_Ip Ip pa2 (Pred m)) pa1 (Pred n) 
                    = alt_Ip (alt_Ip Ip pa1 (Pred n)) pa2 (Pred m) ).
Proof.
  intros.
  apply functional_extensionality.
  intros.
...
Lemma same_alt_rem : forall (W:Set) (l : list predicate)
  ...
  same_Ip_list W Ip Ip' (list_rem l n) <-> 
    same_Ip_list W (alt_Ip Ip pred_asgmt (Pred n)) 
                   (alt_Ip Ip' pred_asgmt (Pred n)) l.
Proof.
  intros W l.
  induction l.
    intros.
...
*)
    ...
    intros.
    apply IHalpha with (Ip := (alt_Ip Ip pred_asgmt (Pred n))).
      apply same_alt_rem.
      apply same_rem.
      apply H.
...
Proof.
  ...
  induction l.
    ...
    apply IHl with (Ip := (alt_Ip Ip pred_asgmt (Pred n))).
      apply same_alt_rem.
      rewrite <- list_without_rem.
      exact H.
